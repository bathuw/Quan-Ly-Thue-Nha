...
namespace QuanLyThueNha01.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        //Admin nhập số tháng vào => sau đó sẽ tính tổng tiền tháng đó 
        [HttpPost("Thong Ke")]
        public async Task<ActionResult> ThongKe(int month)
        {
            var monthInvoices = await _context.Payments.Where(invoice => invoice.PaymentDate.Month == month).ToListAsync();
            decimal totalMonth = 0;

            foreach (var invoice in monthInvoices)
            {
                totalMonth = totalMonth + invoice.TotalAmount;
            }
            return Ok($"Doanh thu cua thang {month} la: {totalMonth}");
        }
